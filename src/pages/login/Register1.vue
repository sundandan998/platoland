<template>
  <div class="register">
    ...
    <div class="register-tab login-form">
      <van-tabs v-model="active" @click="register">
        <van-tab title="手机号注册">
          <mt-field placeholder="请输入手机号" v-model="verification.username" type="tel" :state="telStatus"
            @blur.native.capture="telCheck">
          </mt-field>
          <img src="../../assets/images/tel.svg" alt="" class="login-icon">
        </van-tab>
        <van-tab title="邮箱注册">
          <mt-field placeholder="请输入邮箱" v-model="verification.username" type="email" :state="emailStatus"
            @blur.native.capture="emailCheck">
          </mt-field>
    ...
    <div class="login-form">
      ...
      <!-- @blur.native.capture="invitation" -->
    ...
    watch: {
      ...
      verification: {
        ...
        handler(val) {
          ...
          let email = /^[A-Za-z\d]+([-_.][A-Za-z\d]+)*@([A-Za-z\d]+[-.])+[A-Za-z\d]{2,4}$/
          let password = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,16}$/
          if (val.username != '' && val.password != '' && val.invite_code != '') {
            // 高亮
            this.disabled = false
          } else {
            this.disabled = true
