    ...
    <div class="issued-price">
      <div class="issued-price-top">
        <div class="issued-price-num fl">
          ...
          <span class="fr">{{detail.issue_price|number}}</span>
        </div>
        <span class="fr issued-period issued-price-num-day">{{detail.freeze_days}}</span>
        <div class="issued-price-text fl">
          <span>发行期数 (期)</span>
          <span class="fr">单价 ({{detail.denominated_assets}})</span>
        </div>
        <span class="fr issued-period">期限(天)</span>
      </div>
      <div class="issued-progress progress">
        <mt-progress :value="20" :bar-height="7"></mt-progress>
        <div slot="start" class="fl">已售 {{detail.sold_number|number}} 份</div>
        <div slot="end" class="fr">总量 {{detail.first_number|number}} 份</div>
      </div>
      <mt-cell title="每份数量" value="说明文字"></mt-cell>
      <mt-cell title="每份总价" value="说明文字"></mt-cell>
    </div>
    <div class="issued-servings">
      <mt-cell title="起购份数" value="说明文字"></mt-cell>
      <mt-cell title="最高起购份数" value="说明文字"></mt-cell>
    </div>
    <div class="issued-servings">
      <mt-cell title="股权锁定" :value="detail.equity_issuance_ratio+'%'"></mt-cell>
      <mt-cell title="开始时间" :value="detail.publish_time"></mt-cell>
      <router-link :to="{name:'Issuance',params:{token:detailToken}}">
