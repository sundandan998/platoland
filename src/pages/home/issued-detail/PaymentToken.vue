<template>
  <div class="certificate-list">
    ...
    <div class="token-list" v-for="(item,index) in assetsData" :key="index" v-if="item.subject==null">
        <!-- ,icon:item.icon -->
      <router-link :to="{name:'Token',params:{assets:item.code}}">
...
<script>
  ...
  export default {
    ...
    created() {
      this.list()
      console.log(this.$route.params.assets)
    },
    methods: {
      // 列表信息
      list() {
        ...
        api.tokenList({ category: 'all' }).then(res => {
          ...
          for (let i = 0; i < this.assetsData.length; i++) {
            ...
            if (this.$route.params.assets == this.selectRadio) {
